...
func easyjson8336a586DecodeGithubComMaterkovMeme9Web6Api2(in *jlexer.Lexer, out *ParagraphText) {
	isTopLevel := in.IsStart()
	if in.IsNull() {
		if isTopLevel {
			in.Consumed()
...
func easyjson8336a586EncodeGithubComMaterkovMeme9Web6Api2(out *jwriter.Writer, in ParagraphText) {
	out.RawByte('{')
	first := true
	_ = first
	if in.ID != "" {
	...
	for !in.IsDelim('}') {
		...
		case "paragraphs":
			if in.IsNull() {
				...
				for !in.IsDelim(']') {
					var v1 *InputParagraph
					if in.IsNull() {
						in.Skip()
						v1 = nil
					} else {
						if v1 == nil {
							v1 = new(InputParagraph)
						}
						(*v1).UnmarshalEasyJSON(in)
					}
					out.Paragraphs = append(out.Paragraphs, v1)
					in.WantComma()
				}
				in.Delim(']')
			}
