...
// Server implements the JsonAPI gRPC service
type Server struct {
	json_api.UnimplementedJsonAPIServer
	api *api.API
}
...
func (s *Server) GetFeed(ctx context.Context, req *json_api.FeedRequest) (*json_api.FeedResponse, error) {
	...
	// For feed, auth is optional (only required for subscriptions type)
	...
	if err != nil {
		if err.Error() == "authentication required for subscriptions feed" {
			return nil, status.Error(codes.Unauthenticated, "authentication required for subscriptions feed")
		}
...
func (s *Server) Publish(ctx context.Context, req *json_api.PublishRequest) (*json_api.PublishResponse, error) {
	...
	if err != nil {
		if err.Error() == "text_empty" {
			return nil, status.Error(codes.InvalidArgument, "text_empty")
		}
		if err.Error() == "text_too_long" {
			return nil, status.Error(codes.InvalidArgument, "text_too_long")
		}
		return nil, status.Error(codes.Internal, err.Error())
	}
...
func (s *Server) Login(ctx context.Context, req *json_api.LoginRequest) (*json_api.LoginResponse, error) {
	if req.Username == "" {
		return nil, status.Error(codes.InvalidArgument, "username_required")
	}
	if req.Password == "" {
		return nil, status.Error(codes.InvalidArgument, "password_required")
	}
	...
	if err != nil {
		return nil, status.Error(codes.Internal, "internal_server_error")
	}
...
func (s *Server) Register(ctx context.Context, req *json_api.RegisterRequest) (*json_api.LoginResponse, error) {
	...
	if err != nil {
		if errors.Is(err, users.ErrUsernameExists) {
			return nil, status.Error(codes.AlreadyExists, "username_exists")
		}
		return nil, status.Error(codes.Internal, "internal_server_error")
	}
...
func (s *Server) GetUserPosts(ctx context.Context, req *json_api.UserPostsRequest) (*json_api.UserPostsResponse, error) {
	...
	if err != nil {
		if err.Error() == "user_id is required" {
			return nil, status.Error(codes.InvalidArgument, "user_id_required")
		}
		return nil, status.Error(codes.Internal, err.Error())
	}
